...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        ...
        {
            let mcmd = SubCommand::with_name("batch_enable").about("Enables multiple services on a project. The operation is atomic: if enabling any service fails, then the entire batch fails, and no state changes occur. Operation response type: `google.protobuf.Empty`");
            services0 = services0.subcommand(mcmd);
        }
        {
            let mcmd = SubCommand::with_name("disable").about("Disables a service so that it can no longer be used with a project. This prevents unintended usage that may cause unexpected billing charges or security leaks. It is not valid to call the disable method on a service that is not currently enabled. Callers will receive a `FAILED_PRECONDITION` status if the target service is not currently enabled. Operation response type: `google.protobuf.Empty`");
            services0 = services0.subcommand(mcmd);
        }
        {
            let mcmd = SubCommand::with_name("enable").about("Enables a service so that it can be used with a project. Operation response type: `google.protobuf.Empty`");
            services0 = services0.subcommand(mcmd);
        }
        {
            let mcmd = SubCommand::with_name("generate_service_identity")
                .about("Generates service identity for service.");
            services0 = services0.subcommand(mcmd);
        }
        {
        ...
        {
            let mcmd = SubCommand::with_name("import_admin_overrides").about("Creates or updates multiple admin overrides atomically, all on the same consumer, but on many different metrics or limits. The name field in the quota override message should not be set.");
            consumer_quota_metrics1 = consumer_quota_metrics1.subcommand(mcmd);
        }
        {
            let mcmd = SubCommand::with_name("import_consumer_overrides").about("Creates or updates multiple consumer overrides atomically, all on the same consumer, but on many different metrics or limits. The name field in the quota override message should not be set.");
            consumer_quota_metrics1 = consumer_quota_metrics1.subcommand(mcmd);
        }
        {
