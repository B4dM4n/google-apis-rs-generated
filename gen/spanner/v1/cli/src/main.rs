...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        ...
        let mut databases2 = SubCommand::with_name("databases")
                        .setting(AppSettings::ColoredHelp)
                        .about("methods: create, drop_database, get, get_ddl, get_iam_policy, get_scans, list, restore, set_iam_policy, test_iam_permissions and update_ddl");
        {
        ...
        {
            let mcmd = SubCommand::with_name("drop_database").about("Drops (aka deletes) a Cloud Spanner database. Completed backups for the database will be retained according to their `expire_time`. Note: Cloud Spanner might continue to accept requests for a few seconds after the database has been deleted.");
            databases2 = databases2.subcommand(mcmd);
        }
        {
            let mcmd =
        ...
        {
            let mcmd = SubCommand::with_name("get_scans").about(
