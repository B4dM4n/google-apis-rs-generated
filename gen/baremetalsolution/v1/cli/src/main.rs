...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        let mut app = App::new("baremetalsolution1")
        ...
        let mut instances2 = SubCommand::with_name("instances")
            .setting(AppSettings::ColoredHelp)
            .about("methods: reset_instance");
        {
            let mcmd = SubCommand::with_name("reset_instance").about("Perform an ungraceful, hard reset on a machine (equivalent to shutting the power off, and then turning it back on).");
...
use google_baremetalsolution1 as api;
