...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        let mut app = App::new("firebaseappcheck1")
        ...
        let mut apps1 = SubCommand::with_name("apps")
                        .setting(AppSettings::ColoredHelp)
                        .about("methods: exchange_app_attest_assertion, exchange_app_attest_attestation, exchange_custom_token, exchange_debug_token, exchange_device_check_token, exchange_play_integrity_token, exchange_recaptcha_enterprise_token, exchange_recaptcha_v3_token, exchange_safety_net_token, generate_app_attest_challenge and generate_play_integrity_challenge");
        {
        ...
        {
            let mcmd = SubCommand::with_name("exchange_recaptcha_enterprise_token").about("Validates a [reCAPTCHA Enterprise response token](https://cloud.google.com/recaptcha-enterprise/docs/create-assessment#retrieve_token). If valid, returns an AppCheckToken.");
            apps1 = apps1.subcommand(mcmd);
        }
        {
        ...
        {
            let mcmd = SubCommand::with_name("patch").about("Updates the RecaptchaV3Config for the specified app. While this configuration is incomplete or invalid, the app will be unable to exchange reCAPTCHA tokens for App Check tokens. For security reasons, the `site_secret` field is never populated in the response.");
            recaptcha_v3_config2 = recaptcha_v3_config2.subcommand(mcmd);
        }
...
use google_firebaseappcheck1 as api;
