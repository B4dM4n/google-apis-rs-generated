...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        ...
        {
            let mcmd = SubCommand::with_name("patch_traces").about("Sends new traces to Cloud Trace or updates existing traces. If the ID of a trace that you send matches that of an existing trace, any fields in the existing trace and its spans are overwritten by the provided values, and any new fields provided are merged with the existing trace data. If the ID does not match, a new trace is created.");
        ...
        }
        {
            let mcmd = SubCommand::with_name("list")
                .about("Returns a list of traces that match the specified filter conditions.");
            traces1 = traces1.subcommand(mcmd);
        }
