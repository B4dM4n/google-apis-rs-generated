...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        ...
        {
            let mcmd = SubCommand::with_name("patch").about("Update a `Release` via PATCH. Only updates to `ruleset_name` will be honored. `Release` rename is not supported. To create a `Release` use the CreateRelease method.");
            releases1 = releases1.subcommand(mcmd);
        }
