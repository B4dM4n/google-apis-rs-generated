...
impl<'a, 'b> Default for HeapApp<'a, 'b> {
    fn default() -> Self {
        let mut app = App::new("batch1")
            .setting(clap::AppSettings::ColoredHelp)
            .author("Sebastian Thiel <byronimo@gmail.com>")
            .version("0.1.0-20230118")
            .about("An API to manage the running of batch jobs on Google Cloud Platform.")
        ...
        {
            let mcmd =
                SubCommand::with_name("get").about("Get a Job specified by its resource name.");
            jobs2 = jobs2.subcommand(mcmd);
        }
        {
        ...
        {
            let mcmd =
                SubCommand::with_name("list").about("List all Jobs for a project within a region.");
            jobs2 = jobs2.subcommand(mcmd);
        }
        {
        ...
        let mut state2 = SubCommand::with_name("state")
            .setting(AppSettings::ColoredHelp)
            .about("methods: report");
        {
            let mcmd = SubCommand::with_name("report")
                .about("Report agent's state, e.g. agent status and tasks information");
            state2 = state2.subcommand(mcmd);
        }
        let mut task_groups3 = SubCommand::with_name("task_groups")
            .setting(AppSettings::ColoredHelp)
            .about("sub-resources: tasks");
        let mut tasks4 = SubCommand::with_name("tasks")
            .setting(AppSettings::ColoredHelp)
            .about("methods: get and list");
        {
            let mcmd = SubCommand::with_name("get").about("Return a single Task.");
            tasks4 = tasks4.subcommand(mcmd);
        }
        {
            let mcmd = SubCommand::with_name("list").about("List Tasks associated with a job.");
            tasks4 = tasks4.subcommand(mcmd);
        }
        task_groups3 = task_groups3.subcommand(tasks4);
        jobs2 = jobs2.subcommand(task_groups3);
        locations1 = locations1.subcommand(state2);
